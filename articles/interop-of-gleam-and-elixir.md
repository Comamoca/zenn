...
下準備の前に、mix_gleamがどのような仕組みで動いているのか軽く説明します。
...
次に、`mix deps.get`と`gleam add gleam_stdlib`を実行してください。

その後に`iex -S mix`を実行してみてください。
...
以下はElixirの`IO.puts`関数をGleamで呼び出す例です。
...
- Erlangの場合
`@external(erlang,  "対象の関数が存在しているモジュール名。グローバルに宣言されている場合は省略出来る。", "対象の関数名")`
...
`@external`以下の関数宣言は全て共通です。
`(関数を公開する場合はpubを付ける) fn 関数名(引数) -> 戻り値`
...
というのも、Gleamは一旦Erlangにトランスパイルされた後BEAM(Erlang VM)向けのバイナリ`.beam`ファイルにコンパイルされているからです。[^1]

なので、Elixirから見たGleamファイルは実質Erlangプログラムだったりします。[^2] 

また、この方法で`gleam shell`からプログラムの関数を直接呼び出せたりします。
